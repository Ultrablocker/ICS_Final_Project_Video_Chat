...
class Index:
    ...
    def set_time(self, line):
        self.time_dict[line] = time.strftime('%d.%m.%y,%H:%M', time.localtime())
    ...
    def search_time(self, term):
        msgs = []
        if (term in self.index.keys()):
            indices = self.index[term]
            msgs = [(self.time_dict[i], self.msgs[i]) for i in indices]
        return msgs
...
if __name__ == "__main__":
    ...
    print(sonnets.__dir__())
    p3 = sonnets.get_poem(3)
    # print(p3)
    s_love = sonnets.search_time("love")
    # print(s_love)
